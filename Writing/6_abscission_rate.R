...
# calculate the number of days between abscission events 
bc_data <- 
  read.csv(here('Data/Bird_cage_compiled.csv')) %>%
  as_tibble() %>% 
  filter(stage == 7) %>% #stage 7 = D1 = abscission (remove other stages)
  ...
  mutate(elapsed_days = min_date - min(min_date), # days since first abscission event
         abscission_number = row_number()) %>% # number of abscissions per inflorescence
...
ggplot(data = bc_data, aes(x = as.numeric(elapsed_days), y = abscission_number, color = factor(indiv_ID), group = indiv_ID)) +
  geom_point(size = 4) +
...
ggplot(data = ct_data, aes(x = as.numeric(elapsed_days), y = abscission_number, color = factor(indiv_ID), group = indiv_ID)) +
  geom_point(size = 4) +
  stat_smooth(method = 'lm', fullrange = TRUE, se = FALSE) +
  theme_bw()
...
ggplot(data = alldata, aes(x = as.numeric(elapsed_days), y = abscission_number, group = indiv_treatment)) +
...
bc_models <-
  do(bc_data, glance(lm(abscission_number ~ as.numeric(elapsed_days), data = .)))

bc_models2 <- 
  do(bc_data, tidy(lm(abscission_number ~ as.numeric(elapsed_days), data = .))) %>% 
  filter(term != "(Intercept)") # just interested in slopes


# control models
ct_models <-
  do(ct_data, glance(lm(abscission_number ~ as.numeric(elapsed_days), data = .))) 

ct_models2 <- 
  do(ct_data, tidy(lm(abscission_number ~ as.numeric(elapsed_days), data = .))) %>% 
  filter(term != "(Intercept)") # just interested in slopes
...
# does abscission rate vary between individuals?
summary(aov(slope ~ unique_ID, data = allmodels))

# does abscission rate vary between treatments?
